...
function isJoined($NumberOfTheCourse, $accessCodeArrayed){
    // Add logic from isJoinable here
    if(isAuthorizedToJoin($NumberOfTheCourse, $accessCodeArrayed)){
        return true; // If authorized to join, consider as joined
    }
    
    // Add additional logic from isJoinable if needed
...
echo '<li><strong>'.$course3.' : </strong>'.numberToRankNamed($accessCodeArrayed[3]).displayCourseLink(3,$accessCodeArrayed).' <span class="see
